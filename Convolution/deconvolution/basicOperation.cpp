...
void imageInit(Mat &input,Mat &output)
{
	...
	inputImageMatrix = (double*)fftw_malloc(sizeof(double)* n * m);
	outputImageMatrix = (double*)fftw_malloc(sizeof(double)* n * m);

	inputImageFFT = (fftw_complex*)fftw_malloc(sizeof(fftw_complex)* (n / 2 + 1) * m);
	outputImageFFT = (fftw_complex*)fftw_malloc(sizeof(fftw_complex)* (n / 2 + 1) * m);

	realForwardPlan = fftw_plan_dft_r2c_2d(m, n, inputImageMatrix, inputImageFFT, FFTW_ESTIMATE);//FFTW_ESTIMATE,FFTW_MEASURE
	realBackwardPlan = fftw_plan_dft_c2r_2d(m, n, inputImageFFT, outputImageMatrix, FFTW_ESTIMATE);
...
void psf2otf(Mat &psf, Mat &otf, CvSize size)
{
	...
	Mat q1(I, Rect(cx, 0, cx+1, cy));  // Top-Right
	Mat q2(I, Rect(0, cy, cx, cy+1));  // Bottom-Left
	Mat q3(I, Rect(cx, cy, cx+1, cy+1)); // Bottom-Right

	Mat p0(otf, Rect(n-cx, m-cy, cx, cy));   // Top-Left - Create a ROI per quadrant 
	Mat p1(otf, Rect(0, m-cy, cx+1, cy));  // Top-Right
	Mat p2(otf, Rect(n-cx, 0, cx, cy+1));  // Bottom-Left
	Mat p3(otf, Rect(0, 0, cx+1, cy+1)); // Bottom-Right
...
/******************************************************
*实现2维矩阵的离散傅里叶逆变换，返回一个两通道矩阵
******************************************************/
void ifft2(Mat &srcImage, Mat &dstImage)
{
	Mat padded;
	Mat planes[] = { Mat_<float>(srcImage), Mat::zeros(srcImage.size(), CV_32F) };

	merge(planes, 2, dstImage);
	dft(dstImage, dstImage, DFT_INVERSE + DFT_SCALE);
}
...
void fft2_FFTW(Mat &input, Mat &output)
{
	int m = input.rows;
	int n = input.cols;
	for (int j = 0; j < m; j++)
	{
		uchar *pvalue = input.ptr[j];
		for (int i = 0; i < n; i++)
		{
			&inputImageMatrix[j][i] = pvalue[i];
		}
	}
