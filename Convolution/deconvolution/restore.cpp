...
Mat edgetaper(Mat &image, Mat &psf)
{
	...
	planes2[0].copyTo(temp2);//make sure it is symmetric on both sides
	temp3.copyTo(temp4);
	normalize(beta[0], beta[0], 0, 1, NORM_MINMAX);
	...
	planes3[0] = planes3[0].mul(planes3[0]);
	ifft2(planes3[0], temp1);
	split(temp1, planes3);
	...
	temp6.copyTo(temp7);
	normalize(beta[1], beta[1], 0, 1, NORM_MINMAX);
