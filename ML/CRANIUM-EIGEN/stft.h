...
public:
  ...
  static Eigen::MatrixXcf stft(std::vector<float> &x,
                                                            int n_fft, int n_hop,
                                                            const std::string &win, bool center,
    ...
    Eigen::MatrixXcf r(X.rows(),X.cols());
  ...
  static Eigen::MatrixXf melspectrogram(std::vector<float> &x, int sr, 
    ...
    Eigen::MatrixXf r(mel.rows(),mel.cols());
    for(size_t i = 0; i < mel.rows(); i++)
        for(size_t j = 0; j < mel.cols(); j++)
            r(i,j) = mel_vector[i][j];
    return r;
  }
  ...
  /// \return     mfcc matrix
  static Eigen::MatrixXf mfcc(std::vector<float> &x, int sr,
                                              int n_fft, int n_hop, const std::string &win, bool center, const std::string &mode,
                                              float power, int n_mels, int fmin, int fmax,
    ...
    Eigen::MatrixXf r(dct.rows(),dct.cols());
    for(size_t i = 0; i < dct.rows(); i++)
        for(size_t j = 0; j < dct.cols(); j++)
            r(i,j) = mfcc_vector[i][j];
    return r;
  }
};
