...
struct CuRand 
{
    ...
    void curand_uniform(double * dev, size_t size) {                
        curandGenerateUniform(gen,dev,size*sizeof(double));                
    }
    void curand_normal(double * dev, size_t size, double mean, double stddev) {                
        curandGenerateNormal(gen,dev,size*sizeof(double), mean, stddev);                
    }
    void curand_lognormal(double *dev, size_t size, double mean, double stddev) {                
        curandGenerateLogNormal(gen,dev,size*sizeof(double), mean, stddev);                
    }    
};
...
struct SchemaMemory 
{
    ...
    */
    ...
    double&      f32(size_t pos) { return ((double*)ptr)[pos]; }
    ...
    //double _Complex& cf32(size_t pos) { return ((double _Complex*)ptr)[pos]; }
...
struct DevPtr 
{
    ...
    int    len;
    int    elems;
    DevPtr(void * d, size_t s, size_t e = sizeof(double)) { 
        dev = d; 
        len = s;
        elems = e;         
        host = calloc(s,e);
    ...
    double& f32(size_t i) { return ((double*)host)[i]; }
...
inline void rand(double * devPtr, double *host, int size, double min, double max) {
    ...
    cudaMemcpy(devPtr,host,size*sizeof(double),cudaMemcpyHostToDevice);            
}
...
public:
    ...
    void init(size_t n) {
        dev = find_memory(n);
        if(dev == NULL) {
            cudaMalloc((void**)&dev,n*sizeof(double));
            assert(dev != NULL);
        }
        N    = n;
    ...
    double * dev;
    double * host;
    std::shared_ptr<double> host_ptr;
...
public:
    ...
    Vector(int n, const std::initializer_list<double> & input) {
        init(n);
        std::vector<double> tmp(input.begin(),input.end());
        cudaMemcpy(dev,tmp.data(),n*sizeof(double),cudaMemcpyHostToDevice);
    }
    Vector(int n, const std::vector<double> & tmp) {
        init(n);        
        cudaMemcpy(dev,tmp.data(),n*sizeof(double),cudaMemcpyHostToDevice);
    }    
    ...
    void download_host()     {
        cudaMemcpy(host,dev,size()*sizeof(double),cudaMemcpyDeviceToHost);
    }
    void upload_device()     {
        cudaMemcpy(dev,host,size()*sizeof(double),cudaMemcpyHostToDevice);
    }    
    void zero()    {
        cudaMemsetAsync(dev,0x00,size()*sizeof(double));
    }
    void ones()    {
        fill(1.0f);
    }    
    ...
    double& operator[](array_index pos)    {        
        assert(pos < size());
        while(pos < 0) pos += size();
        return host[pos];        
    }
    double __getitem(array_index pos) { return (*this)[pos]; }
    void __setitem(array_index pos, double val) { while(pos < 0) pos += N; host[pos] = val; }
    ...
    /*
    void copysign(const double val) {
...
struct VectorView {
    ...
    VectorView(double *h, int row, int cols) {        
        host = h;
        r = row;
        c = cols;
    }
    ...
    double __getitem(array_index pos) {
        while(pos < 0) pos += c;
        return host[r*c+pos];
    }
    void  __setitem(array_index pos, double val) {
        while(pos < 0) pos += c;
        host[r*c+pos] = val;                
    }
};
...
public:
    ...
    void init(size_t m, size_t n) {        
        dev = find_memory(m*n);
        if(dev == NULL) {
            cudaMalloc((void**)&dev,m*n*sizeof(double));
            assert(dev != NULL);
        }        
        M    = m;
        N    = n;        
        host = (double*)calloc(m*n,sizeof(double));
        assert(host != NULL);
        host_ptr = { host, free };
...
public:
    ...
    Matrix(double * p, size_t m, size_t n) {
        M = m;
        N = n;
        host = (double*)calloc(m*n,sizeof(double));
        host_ptr = { host, free };
    ...
    Matrix(size_t i, size_t j, const std::initializer_list<double> & in) {
        init(i,j);
        std::vector<double> v(in.begin(),in.end());
        cudaMemcpy(dev,v.data(), size()*sizeof(double),cudaMemcpyHostToDevice);
    }
    Matrix(size_t i, size_t j, const std::vector<double> & v) {
        init(i,j);
        cudaMemcpy(dev,v.data(), size()*sizeof(double),cudaMemcpyHostToDevice);
    }    
    ...
    Matrix get_row(int row) {
        ...
        assert(row < M);   
        Matrix r(1,cols());        
        cudaMemcpy(r.dev,dev+row*N,cols()*sizeof(double),cudaMemcpyDeviceToDevice);
        return r;
    }
    void set_row(const Matrix & m, int dst_row=0, int src_row=0) {
        ...
        cudaMemcpy(dev + dst_row*N, m.dev + src_row*m.N, m.N*sizeof(double), cudaMemcpyDeviceToDevice);
    }
    void set_row(const Vector & v, int dst_row=0) {
        assert(N == v.N);       
        while(dst_row < 0) dst_row += M; 
        cudaMemcpy(dev + dst_row*N, v.dev, v.N*sizeof(double), cudaMemcpyDeviceToDevice);
    }
...
inline Vector matvec(const Matrix & a, const Vector & v,bool transa, double alpha=1.0, double beta=0.0) {    
    Vector R(v.size());
    int m = a.M;
    int n = a.N;
...
inline Matrix matmul(const Matrix & a, const Matrix & b, double alpha=1.0, double beta=0.0) {    
    assert(a.N == b.M);     
    Matrix C(a.M,b.N);
    cublasOperation_t ta = CUBLAS_OP_N;
...
inline Matrix matmulTT(const Matrix & a,const Matrix & b, double alpha=1.0, double beta=0.0) {         
    assert(a.N == b.M);    
...
inline Matrix matmulTN(const Matrix & a,const Matrix & b, double alpha=1.0, double beta=0.0) {         
    assert(a.N == b.N);    
    cublasOperation_t ta = CUBLAS_OP_N;
    cublasOperation_t tb = CUBLAS_OP_T;               
    Matrix C(a.N,b.N);        
...
// C^T = B*A^T
// A is in column major order
inline Matrix matmulNT(const Matrix & a,const Matrix & b, double alpha=1.0, double beta=0.0) {         
    assert(a.M == b.M);    
    cublasOperation_t ta = CUBLAS_OP_T;
    cublasOperation_t tb = CUBLAS_OP_N;            
    Matrix C(a.M,b.M);        
...
inline Matrix operator + (const Matrix & a, const double v)
{    
    Matrix r(a.M,a.N);        
...
inline Matrix operator + (const double v, const Matrix & a)
{
    Matrix r(a.M,a.N);    
    //matrix_r_addf_const(a.dev,v,r.dev,a.M,a.N);
    vector_r_addf_const(a.dev,v,r.dev,a.M*a.N);
...
inline Matrix operator / (const Matrix & a, double v)
{    
    Matrix r = CopyMatrix(a);
    assert(v != 0);
    v= 1.0/v;
...
inline Matrix pow(const Matrix & m,double p) {
    Matrix r = CopyMatrix(m);
    r.pow(p);
    return r;
}
...
inline Matrix atan2(const Matrix & m,double v) {
    Matrix r = CopyMatrix(m);
    r.atan2(v);
    return r;
}
...
/*
inline Matrix copysign(const Matrix & m, double x) {
    Matrix r = CopyMatrix(m);
    r.copysign(x);
    return r;
}
...
inline Vector pow(const Vector & m,double p) {
    Vector r = CopyVector(m);
    r.pow(p);
    return r;
}
...
inline Vector atan2(const Vector & m,double v) {
    Vector r = CopyVector(m);
    r.atan2(v);
    return r;
}
...
/*
inline Vector copysign(const Vector & m, double x) {
    Vector r = CopyVector(m);
    r.copysign(x);
    return r;
}
