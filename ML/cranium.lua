...
function connection_InitializeConnection(connection)
    ...
    connection.weights:upload_device()
end 
...
function network_forwardPass(network,input)               
    ...
    for i=0,network.numConnections-1 do                                                  
        ...
        tmp:addToEachRow(network.connections[i].bias)                       
        network.connections[i].to.input = tmp:eval()
...
function network_meanSquaredError(network, prediction, actual, regularizationStrength)
    ...
    total_err = vf.hadamard(tmp,tmp):sum()
    for i=0,network.numConnections - 1 do 
        local w = network.connections[i].weights    
        reg_err = reg_err + vf.hadamard(w,w):sum()
    end 
...
function generate_index(batches,batchSize, data,classes,shuffle)
    ...
    for i=0,batchSize-1 do         
        r[i] = {}
        r[i][1] = vf.Matrix(data:row(i))
        r[i][2] = vf.Matrix(classes:row(i))                
    end 
    if(shuffle) then 
        r = rng.shuffle(r)
    end 
...
function generate_batch(numBatches,batchSize, data,classes,shuffle)
    ...
    for i=0,numBatches-1 do 
        ...
        for j=0,curBatchSize-1 do                         
            l[j] = {}                    
            local example = data:get_row(rc)        
            local class   = classes:get_row(rc)
            l[j][1] = vf.Matrix(example)
            l[j][2] = vf.Matrix(class)
            rc = rc + 1 
            rc = rc % data:rows()                
        end         
        r[i] = l
            ...
            for training=0,curBatchSize-1 do                            
                ...
                for layer=network.numLayers-1,1,-1 do                                         
                    ...
                    if(layer == network.numLayers-1) then                                                                                                                          
                        ...
                        wTi[hiddenLayer] = network.connections[layer].weights:t()
                        ...
                        errori[layer] = vf.hadamard(errorLastTi[hiddenLayer],fprimei[hiddenLayer])                                                                                            
                        inputTi[hiddenLayer] = con.from.input:t()
...
function XOR(f)
    ...
    net = network_new(2,1,{16},{f},1,linear)
