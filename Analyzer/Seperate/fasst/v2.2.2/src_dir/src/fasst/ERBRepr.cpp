...
ERBRepr::ERBRepr(const Audio &x, int wlen, int nbin) {
  ...
  int N = static_cast<int>(std::ceil(static_cast<double>(samples) / wlen * 2));
  ...
  for (int i = 0; i < I; i++) {
    ...
    VectorXcd fchan;
    fft.fwd(fchan, xchan);
    int index = static_cast<int>(std::ceil(static_cast<double>(N + 1) * wlen / 4.) - 1);
  ...
  for (int f = F - 1; f >= 0; f--) {
    ...
    // Bandpass filter
    int hwlen = static_cast<int>(std::round((a(f) / subs(f))));
    ...
    for (int n = 0; n < N; n++) {
      ...
      m_X(f, n) = (xxfram.adjoint() * xxfram).conjugate() * subs(f) / std::pow(hwlen + 1., 2.);
    }
  }
}
  void ERBRepr::FilterERB(const Audio &x, int wlen, const Sources * srcs, const std::string &dirName) {

  int samples = x.samples();
  double fs = x.samplerate();
  int I = x.channels();
  int J = static_cast<int>(srcs->sources());
  int F = srcs[0].bins();
  const std::complex<double> M_I(0, 1);
  ...
  for (int f = 0; f < F; f++) {
    subs(f) = std::round(subs(f));
  } // after this loop, subs can be casted to int
  ArrayXd subs_shift(F);
  subs_shift.segment(0, F-1) = subs.tail(F-1);
  subs_shift(F-1) = 1.;
  ...
  ArrayXXd yy = ArrayXXd::Zero((N + 1) * wlen / 2, I*J);
  ArrayXXcd yyscale = ArrayXXcd::Zero((N + 1) * wlen / 2, I*J);
  ...
  for (int n = 0; n < N; n++) {
	  for (int f = 0; f < F; f++) {
		  ...
		  MatrixXcd Sigma_x = MatrixXcd::Identity(I, I)*srcs->getNoiseEnd(f);
		  for (int j = 0; j < J; j++) {
			  Sigma_x += ((*srcs)[j]).compSigmaY(f, n);
		  }
  ...
  for (int f = F - 1; f >= 0; f--) {
    ...
    cout << "      Wiener filtering of frequency band " << F-f <<"/"<< F << " for all sources" << endl;
    ...
    if (subs(f) != subs_shift(f)) {
      ...
      swin = Eigen::sqrt(swin);
      yy += upsample(yyscale, static_cast<int>(subs(f+1))).real();
      yyscale = ArrayXXcd::Zero((N + 1) * wlen / 2, I*J);
    }
    ...
    // Bandwise filtering
    ArrayXXcd yyband = ArrayXXcd::Zero((N + 1) * wlen / 2, I*J);
    for (int j = 0; j < J; j++) {
        for (int n = 0; n < N; n++) {
            MatrixXcd xxfram(wlen, I);
            for (int i = 0; i < I; i++) {
            ...
            MatrixXcd W = Source::WienerThresholding((*srcs)[j].compSigmaY(f, n)*Sigma_x_inverse(f, n), (*srcs)[j].wiener_qd());
            yyband.block(n * wlen / 2, j*I, wlen, I) += (xxfram * W.transpose()).array();
        }
    }
  ...
  yy += upsample(yyscale, static_cast<int>(subs(0))).real();
 
  // Seeing output as audio
  vector<Audio> output(J);
  for (int j = 0; j < J; j++) {
    ArrayXXd y = yy.block(0, j*I, samples, I);
    Audio(y, x.samplerate()).write(dirName + (*srcs)[j].name() + ".wav", x.samplerate());
  }
 
}

Eigen::ArrayXXcd ERBRepr::fftfilt(Eigen::ArrayXcd h, Eigen::ArrayXXcd x) {
  int samples = static_cast<int>(x.rows());
  int I = static_cast<int>(x.cols());
  int L = static_cast<int>(h.rows());
  ...
  // FFT size
  int nfft = static_cast<int>(std::pow(2, std::ceil(std::log(static_cast<double>(samples + L - 1))/std::log(2.))));
