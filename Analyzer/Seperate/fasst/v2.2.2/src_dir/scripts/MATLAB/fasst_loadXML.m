...
% Read wlen and nbin (if nbin exist: mandatory only for ERB use case)
data.wlen = str2num(domnode.getElementsByTagName('wlen').item(0).getTextContent);
if ~isempty(domnode.getElementsByTagName('nbin').item(0))
    data.nbin = str2num(domnode.getElementsByTagName('nbin').item(0).getTextContent);
end

if ~isempty(domnode.getElementsByTagName('nbinPerERB_ERBLET').item(0))
    data.nbinPerERB_ERBLET = str2num(domnode.getElementsByTagName('nbinPerERB_ERBLET').item(0).getTextContent);
end

% Read iterations number
