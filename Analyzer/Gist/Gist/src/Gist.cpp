...
void Gist<T>::configureFFT()
{
    ...
    fft_real_out = (double*)fftw_malloc(sizeof(double)*frameSize);
    
    // FFT plan initialisation
    p = fftw_plan_dft_1d (frameSize, fftIn, fftOut, FFTW_FORWARD, FFTW_ESTIMATE);
    synth = fftw_plan_dft_c2r_1d(frameSize, fftIn, fft_real_out, FFTW_ESTIMATE);
#endif /* END USE_FFTW */
...
void Gist<T>::freeFFT()
{
    ...
    // destroy fft plan
    fftw_destroy_plan (p);
    fftw_destroy_plan (synth);
    fftw_free (fftIn);
    fftw_free (fftOut);
    fftw_free (fft_real_out);
#endif
...
void Gist<T>::performFFT()
{
    ...
    /* you can but it sounds like itchy scratchies
    if(ola.size() > 0) {
        for(int i = 0; i < hop_size; i++) {
            fftIn[i][0] += ola[i].real();
            fftIn[i][1] += ola[i].imag();
        }
        ola.erase(ola.begin(),ola.begin()+hop_size);
    }
    */
    // perform the FFT
    fftw_execute (p);
    ...
    for (int i = 0; i < frameSize; i++)
    {
        fftReal[i] = (T)fftOut[i][0]/(T)frameSize;
        fftImag[i] = (T)fftOut[i][1]/(T)frameSize;
        //ola.push_back(std::complex<T>(fftReal[i],fftImag[i]));
    }
#endif
...
template<class T>
std::vector<T> Gist<T>::synthesize()
{
#ifdef USE_FFTW
    // copy samples from audio frame
    for (int i = 0; i < frameSize; i++)
    {
        fftIn[i][0] = (double)fftReal[i];
        fftIn[i][1] = (double)fftImag[i];
    }
    memset(fft_real_out,0,frameSize*sizeof(double));
    // perform the FFT
    fftw_execute (synth);
    
    std::vector<T> out(frameSize);    
    for(size_t i = 0; i < frameSize; i++)
    {
        out[i] = 0.5*fft_real_out[i];
    }
